...
public sealed class GeoLogController : ControllerBase
{
    ...
    public GeoLogController(IConfiguration configuration) => _configuration = configuration;
...
RETURNING ""Id"";";
        ...
        var corrIdHdr = Request.Headers["X-Correlation-Id"].ToString();
        var correlationId = string.IsNullOrWhiteSpace(corrIdHdr) ? HttpContext.TraceIdentifier : corrIdHdr;
        ...
        return Created($"/api/geolog/{newId}", new { id = newId });
    }
}
